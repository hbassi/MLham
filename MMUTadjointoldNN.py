...
def newgradflattener(td):
    ...
    for key in allkeys:
        subkeys = td[key].keys()
        for subkey in subkeys:
            alldata.append(td[key][subkey].flatten())
    return jnp.concatenate(alldata)
#============================================================================================================#
...
def MMUT_Prop_HSB(theta, initial_density, tmeoff=1, fldfrq=1, fldamp=1, norm_direc=jnp.array([0.0,0.0,1.0])):
    tvec = dt*jnp.arange(ntvec)
...
def loss(theta, thisden, thistmeoff, thisfrq, thisamp, thisdirec):
...
mlprop = MMUT_Prop_HSB(theta0, ic, tmeoff=alltme[i], fldfrq=allfrq[i], fldamp=allamp[i])
